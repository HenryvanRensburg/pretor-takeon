            ...
            if sub_nav == "Overview":
                ...
                if not items.empty:
                        ...
                        if not df_pending.empty:
                            ...
                            if not ag_pend.empty:
                                ...
                                if st.button("Save Agent Items"):
                                    ...
                                    save_checklist_batch(b_choice, edited_ag); st.cache_data.clear(); st.success("Saved!"); st.rerun()
                                st.divider()
                             ...
                             if not ag_comp.empty:
                                 ...
                                 # CLIENT COMPLETION LOGIC (MOVED HERE)
                                 st.divider()
                                 ...
                                 if st.button("📄 Create PDF Report", key="gen_pdf_comp"):
                                     pdf_f = create_comprehensive_pdf(b_choice, p_row, c_items, emp_df, arr_df, cou_df)
                                     with open(pdf_f, "rb") as f: st.download_button("⬇️ Download PDF", f, file_name=pdf_f, mime="application/pdf")

                                 # STEP 2: EMAIL
                                 st.markdown("**Step 2: Email Client**")
                                 if comp_date and comp_date != "None":
                                     st.success(f"✅ Sent: {comp_date}")
                                     if st.button("Unlock (Resend)", key="unlock_comp"):
                                         update_email_status(b_choice, "Client Completion Email Sent Date", ""); st.cache_data.clear(); st.rerun()
                                 else:
                                     ...
                                     if c_mail and c_mail != "None":
                                         ...
                                         if st.button("Mark as Sent", key="mark_comp"):
                                              update_email_status(b_choice, "Client Completion Email Sent Date"); st.cache_data.clear(); st.rerun()
                                     else: st.warning("No Client Email.")
                             else: st.info("No agent items.")

                    with t2:
                         # Internal Logic Same as before
                         if not df_pending.empty:
                            ...
                            if not int_pend.empty:
                                ...
                                if st.button("Save Internal Items"):
                                    ...
                                    save_checklist_batch(b_choice, ed_int); st.cache_data.clear(); st.success("Saved!"); st.rerun()
                    ...
                    if not df_completed.empty:
                        ...
                        with h1: st.dataframe(ah[['Task Heading', 'Task Name', 'Date Received', 'Notes']], hide_index=True, use_container_width=True)
                        with h2: st.dataframe(ih[['Task Heading', 'Task Name', 'Date Received', 'Notes']], hide_index=True, use_container_width=True)
                else: st.info("No checklist.")

            elif sub_nav == "Staff Details":
                # (Code same as previous - abbreviated for brevity but please keep full logic)
                ...
                if locked:
                    ...
                    with st.form("stat"):
                        ...
                        if st.form_submit_button("Save"): update_building_details_batch(b_choice, {"UIF Number": u, "PAYE Number": p, "COIDA Number": c}); st.cache_data.clear(); st.rerun()
                ...
                if not all_s.empty and 'Complex Name' in all_s.columns:
                    ...
                    if not curr_s.empty:
                        ...
                        ed_s = st.data_editor(curr_s[[c for c in cols if c in curr_s.columns]], hide_index=True, key="stf_ed", column_config={"id": None})
                        if st.button("Save Staff"): update_employee_batch(ed_s); st.cache_data.clear(); st.rerun()
                ...
                with st.form("add_s", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Add"): add_employee(b_choice, n, s, "", "", 0.0, False, False, False); st.cache_data.clear(); st.rerun()
                ...
                if not ad.empty and 'Complex Name' in ad.columns:
                    ...
                    if not curr_a.empty:
                         ed_a = st.data_editor(curr_a[['id', 'Unit Number', 'Outstanding Amount']], hide_index=True, key="arr_ed", column_config={"id": None})
                         if st.button("Save Arrears"): update_arrears_batch(ed_a); st.cache_data.clear(); st.rerun()
                    else: st.info("No arrears.")
                with st.form("add_a", clear_on_submit=True):
                    u=st.text_input("Unit"); a=st.number_input("Amount")
                    if st.form_submit_button("Add"): add_arrears_item(b_choice, u, a, "", "", ""); st.cache_data.clear(); st.rerun()
                ...
                if not cd.empty and 'Complex Name' in cd.columns:
                    ...
                    if not curr_c.empty:
                        ...
                        if st.button("Save Council"): update_council_batch(ed_c); st.cache_data.clear(); st.rerun()
                    else: st.info("No accounts.")
                with st.form("add_c", clear_on_submit=True):
                    a=st.text_input("Acc"); s=st.text_input("Svc")
                    if st.form_submit_button("Add"): add_council_account(b_choice, a, s, 0.0); st.cache_data.clear(); st.rerun()

            elif sub_nav == "Department Handovers":
                st.markdown("### Department Handovers")
                # (Same logic as before, shortened for block fit)
                # ... [Include SARS, Council, Wages, Insurance, Debt logic from previous step] ...
                # Just ensuring the structure is maintained:
                st.info("Manage individual department emails here (Wages, SARS, etc).")
