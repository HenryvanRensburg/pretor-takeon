...
def generate_appointment_pdf(complex_name, checklist_df, agent_name, take_on_date, immediate_items_list):
    ...
    # Strict Filter: Remove anything that might be internal
    agent_only_df = checklist_df[checklist_df['Responsibility'].astype(str).str.contains('Agent|Both', case=False, na=False)]

    # Split
    df_immediate = agent_only_df[agent_only_df['Task Name'].isin(immediate_items_list)]
    df_month_end = agent_only_df[~agent_only_df['Task Name'].isin(immediate_items_list)]
...
def create_comprehensive_pdf(complex_name, p_row, checklist_df, emp_df, arrears_df, council_df):
    ...
    pdf.section_title("2. Items Received"); pdf.ln(2)
    if not checklist_df.empty:
        rec = checklist_df[(checklist_df['Responsibility'].astype(str).str.contains('Agent|Both', case=False, na=False)) & (checklist_df['Received'].astype(str).str.lower() == 'true')]
        if not rec.empty:
            for _, r in rec.iterrows(): pdf.cell(5); pdf.multi_cell(0, 5, f"- {pdf.clean_text(r['Task Name'])} (Rec: {r.get('Date Received','')})")
    ...
    # --- MANAGE BUILDINGS ---
        ...
        if sub_nav == "Overview":
            ...
            if agent_task_df.empty:
                ...
                # Find items that are NOT month-end (default immediate)
